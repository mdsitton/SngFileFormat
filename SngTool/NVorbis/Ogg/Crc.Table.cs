...
namespace NVorbis.Ogg
{
    internal partial struct Crc
    {
        ...
        static Crc()
        {
            ...
            uint[] crcTable = new uint[TableLength * 8];
            Init(crcTable, false, 32, CRC32_POLY);
            if (!s_crcTable.AsSpan().SequenceEqual(crcTable))
        ...
        private static int Init(Span<uint> table, bool le, int bits, uint poly)
        {
            ...
            {
                table[TableLength] = 1;
                int tables = table.Length / TableLength - 1;

                for (int i = 0; i < TableLength; i++)
                {
                    for (int j = 0; j < tables; j++)
                    {
                        table[TableLength * (j + 1) + i] =
                            (table[TableLength * j + i] >> 8) ^ table[(int)(table[TableLength * j + i] & 0xFF)];
                    }
                }
            }
            return 0;
        ...
        private static readonly uint[] s_crcTable = new uint[TableLength * 8]
        {
